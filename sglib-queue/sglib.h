...
#define SGLIB_ARRAY_SINGLE_HEAP_SORT(type, a, max, comparator) \
  {                                                            \
    SGLIB_ARRAY_HEAP_SORT(type, a, max, comparator,            \
...
#define SGLIB_ARRAY_BINARY_SEARCH(type, a, start_index, end_index, key, \
                                  comparator, found, result_index)      \
  {                                                                     \
...
#define SGLIB_HASH_TAB_ADD_IF_NOT_MEMBER(type, table, dim, elem,            \
                                         hash_function, comparator, member) \
  {                                                                         \
    unsigned _pos_;                                                         \
    type *_elem_;                                                           \
...
#define SGLIB_HASH_TAB_FIND_MEMBER(type, table, dim, elem, hash_function,  \
                                   comparator, resultIndex, resultMember)  \
  {                                                                        \
    unsigned _i_;                                                          \
    int _count_;                                                           \
    type *_e_;                                                             \
...
#define SGLIB_HASH_TAB_MAP_ON_ELEMENTS(type, table, dim, iteratedIndex, \
                                       iteratedVariable, command)       \
  {                                                                     \
...
#define SGLIB_LIST_DELETE(type, list, elem, next)                            \
  {                                                                          \
    ...
    for (_p_ = &(list); *_p_ != NULL && *_p_ != (elem); _p_ = &(*_p_)->next) \
      ;                                                                      \
    assert(*_p_ != NULL &&                                                   \
           "element is not member of the container, use DELETE_IF_MEMBER "   \
           "instead" != NULL);                                               \
    *_p_ = (*_p_)->next;                                                     \
  }
...
#define SGLIB_LIST_SORT(type, list, comparator, next)                 \
  {                                                                   \
    ...
    for (_n_ = 1; _contFlag_; _n_ = _n_ + _n_) {                      \
      ...
      while (_todo_ != NULL) {                                        \
        _a_ = _todo_;                                                 \
        for (_i_ = 1, _t_ = _a_; _i_ < _n_ && _t_ != NULL;            \
             _i_++, _t_ = _t_->next)                                  \
          ;                                                           \
        if (_t_ == NULL) {                                            \
          *_restail_ = _a_;                                           \
        ...
        _b_ = _t_->next;                                              \
        _t_->next = NULL;                                             \
        for (_i_ = 1, _t_ = _b_; _i_ < _n_ && _t_ != NULL;            \
             _i_++, _t_ = _t_->next)                                  \
          ;                                                           \
        if (_t_ == NULL) {                                            \
          _todo_ = NULL;                                              \
...
#define SGLIB_SORTED_LIST_ADD(type, list, elem, comparator, next)              \
  {                                                                            \
    type **_e_;                                                                \
    int _cmpres_;                                                              \
    SGLIB_SORTED_LIST_FIND_MEMBER_OR_PLACE(type, list, elem, comparator, next, \
                                           _cmpres_, _e_);                     \
    (elem)->next = *_e_;                                                       \
    *_e_ = (elem);                                                             \
  }

#define SGLIB_SORTED_LIST_ADD_IF_NOT_MEMBER(type, list, elem, comparator,      \
                                            next, member)                      \
  {                                                                            \
    type **_e_;                                                                \
    int _cmp_res_;                                                             \
...
#define SGLIB_SORTED_LIST_FIND_MEMBER(type, list, elem, comparator, next, \
  ...
  {                                                                       \
    ...
    int _cmpres_ = 1;                                                     \
    for (_p_ = (list);                                                    \
         _p_ != NULL && (_cmpres_ = comparator(_p_, (elem))) < 0;         \
         _p_ = _p_->next)                                                 \
      ;                                                                   \
    if (_cmpres_ != 0)                                                    \
      (member) = NULL;                                                    \
...
#define SGLIB___DL_LIST_GENERIC_ADD_IF_NOT_MEMBER(                           \
  ...
  {                                                                          \
    ...
    if (_dlp_ == NULL && (list) != NULL) {                                   \
      for (_dlp_ = (list)->next;                                             \
           _dlp_ != NULL && comparator(_dlp_, (elem)) != 0;                  \
           _dlp_ = _dlp_->next)                                              \
        ;                                                                    \
    }                                                                        \
    (member) = _dlp_;                                                        \
...
#define SGLIB_DL_LIST_ADD_BEFORE_IF_NOT_MEMBER(type, list, elem, comparator, \
                                               previous, next, member)       \
  {                                                                          \
    SGLIB___DL_LIST_GENERIC_ADD_IF_NOT_MEMBER(type, list, elem, comparator,  \
                                              previous, next, member,        \
                                              SGLIB_DL_LIST_ADD_BEFORE);     \
  }

#define SGLIB_DL_LIST_ADD_AFTER_IF_NOT_MEMBER(type, list, elem, comparator, \
                                              previous, next, member)       \
  {                                                                         \
    SGLIB___DL_LIST_GENERIC_ADD_IF_NOT_MEMBER(type, list, elem, comparator, \
                                              previous, next, member,       \
...
#define SGLIB_DL_LIST_ADD_IF_NOT_MEMBER(type, list, elem, comparator,       \
                                        previous, next, member)             \
  {                                                                         \
    SGLIB___DL_LIST_GENERIC_ADD_IF_NOT_MEMBER(type, list, elem, comparator, \
                                              previous, next, member,       \
...
#define SGLIB_DL_LIST_DELETE_IF_MEMBER(type, list, elem, comparator, previous, \
                                       next, member)                           \
  {                                                                            \
    type *_dlp_;                                                               \
    for (_dlp_ = (list); _dlp_ != NULL && comparator(_dlp_, (elem)) != 0;      \
...
#define SGLIB_DL_LIST_FIND_MEMBER(type, list, elem, comparator, previous,   \
                                  next, member)                             \
  {                                                                         \
    type *_dlp_;                                                            \
...
#define SGLIB_DL_LIST_MAP_ON_ELEMENTS(type, list, iteratedVariable, previous,  \
                                      next, command)                           \
  {                                                                            \
    type *_dl_;                                                                \
    type *iteratedVariable;                                                    \
    if ((list) != NULL) {                                                      \
      _dl_ = (list)->next;                                                     \
      SGLIB_LIST_MAP_ON_ELEMENTS(type, list, iteratedVariable, previous,       \
                                 command);                                     \
...
#define SGLIB___BIN_TREE_MAP_ON_ELEMENTS(type, tree, iteratedVariable, order,  \
                                         left, right, command)                 \
  {                                                                            \
    ...
    while (_cn_ != NULL) {                                                     \
      ...
      do {                                                                     \
        ...
        if ((order == 1 && _pass_[_pathi_] == 0) ||                            \
            (order == 2 &&                                                     \
             (_pass_[_pathi_] == 1 || _right_[_pathi_] == NULL))) {            \
          iteratedVariable = _path_[_pathi_];                                  \
          { command; }                                                         \
        }                                                                      \
        _pass_[_pathi_]++;                                                     \
...
#define SGLIB_BIN_TREE_MAP_ON_ELEMENTS(type, tree, _current_element_, left,  \
                                       right, command)                       \
  {                                                                          \
    SGLIB___BIN_TREE_MAP_ON_ELEMENTS(type, tree, _current_element_, 1, left, \
                                     right, command);                        \
  }

#define SGLIB_BIN_TREE_MAP_ON_ELEMENTS_PREORDER(type, tree, _current_element_, \
                                                left, right, command)          \
  {                                                                            \
    SGLIB___BIN_TREE_MAP_ON_ELEMENTS(type, tree, _current_element_, 0, left,   \
                                     right, command);                          \
  }
...
#define SGLIB_BIN_TREE_MAP_ON_ELEMENTS_POSTORDER(                            \
  ...
  {                                                                          \
    SGLIB___BIN_TREE_MAP_ON_ELEMENTS(type, tree, _current_element_, 2, left, \
                                     right, command);                        \
  }

#define SGLIB___BIN_TREE_FIND_MEMBER(type, tree, elem, left, right, \
                                     comparator, res)               \
  {                                                                 \
    type *_s_;                                                      \
    int _c_;                                                        \
...
#define SGLIB_DEFINE_QUEUE_PROTOTYPES(queue_type, elem_type, afield, ifield, \
                                      jfield, dim)                           \
...
#define SGLIB_DEFINE_QUEUE_FUNCTIONS(queue_type, elem_type, afield, ifield,    \
                                     jfield, dim)                              \
  ...
  int sglib_##queue_type##_is_full(queue_type *q) {                            \
    return (                                                                   \
        SGLIB_QUEUE_IS_FULL(elem_type, q->afield, q->ifield, q->jfield, dim)); \
  }                                                                            \
  elem_type sglib_##queue_type##_first_element(queue_type *q) {                \
    return (SGLIB_QUEUE_FIRST_ELEMENT(elem_type, q->afield, q->ifield,         \
                                      q->jfield));                             \
  }                                                                            \
...
#define SGLIB_DEFINE_HEAP_PROTOTYPES(heap_type, elem_type, afield, ifield, \
                                     dim, comparator, elem_exchanger)      \
...
#define SGLIB_DEFINE_HEAP_FUNCTIONS(heap_type, elem_type, afield, ifield, dim, \
                                    comparator, elem_exchanger)                \
  ...
  void sglib_##heap_type##_add_next(heap_type *q) {                            \
    SGLIB_HEAP_ADD_NEXT(elem_type, q->afield, q->ifield, dim, comparator,      \
                        elem_exchanger);                                       \
  }                                                                            \
  void sglib_##heap_type##_add(heap_type *q, elem_type elem) {                 \
    SGLIB_HEAP_ADD(elem_type, q->afield, elem, q->ifield, dim, comparator,     \
                   elem_exchanger);                                            \
  }                                                                            \
  void sglib_##heap_type##_delete_first(heap_type *q) {                        \
    SGLIB_HEAP_DELETE_FIRST(elem_type, q->afield, q->ifield, dim, comparator,  \
                            elem_exchanger);                                   \
  }                                                                            \
...
/*
  ...
  sglib's hash table is an array storing directly pointers to objects (not
  containers). In this table there is a one-to-one mapping between 'objects'
  stored in the table and indexes where they are placed. Each index is pointing
  to exactly one 'object' and each 'object' stored in the table occurs on
  exactly one index.  Once an object is stored in the table, it can be
  represented via its index.
...
#define SGLIB_DEFINE_HASHED_TABLE_PROTOTYPES(type, dim, hash_function,      \
                                             comparator)                    \
  ...
  extern type *sglib_hashed_##type##_find_member(type *table[dim],          \
                                                 type *elem);               \
  extern type *sglib_hashed_##type##_it_init(                               \
  ...
  extern type *sglib_hashed_##type##_it_current(                            \
      struct sglib_hashed_##type##_iterator *it);                           \
  extern type *sglib_hashed_##type##_it_next(                               \
      struct sglib_hashed_##type##_iterator *it);

#define SGLIB_DEFINE_HASHED_TABLE_FUNCTIONS(type, dim, hash_function,       \
                                            comparator)                     \
  struct sglib_hashed_##type##_iterator {                                   \
    int currentIndex;                                                       \
    type **table;                                                           \
  ...
  int sglib_hashed_##type##_add_if_not_member(type *table[dim], type *elem, \
                                              type **member) {              \
    SGLIB_HASH_TAB_ADD_IF_NOT_MEMBER(type, table, dim, elem, hash_function, \
                                     comparator, *member);                  \
  }                                                                         \
  ...
  type *sglib_hashed_##type##_find_member(type *table[dim], type *elem) {   \
    type *mmb;                                                              \
    int ind;                                                                \
    SGLIB_HASH_TAB_FIND_MEMBER(type, table, dim, elem, hash_function,       \
                               comparator, ind, mmb);                       \
    return (mmb);                                                           \
  }                                                                         \
  ...
  type *sglib_hashed_##type##_it_init(                                      \
      struct sglib_hashed_##type##_iterator *it, type *table[dim]) {        \
    sglib_hashed_##type##_it_init_on_equal(it, table, NULL, NULL);          \
  }                                                                         \
  type *sglib_hashed_##type##_it_current(                                   \
      struct sglib_hashed_##type##_iterator *it) {                          \
    return (table[it->currentIndex]);                                       \
  }                                                                         \
  type *sglib_hashed_##type##_it_next(                                      \
      struct sglib_hashed_##type##_iterator *it) {                          \
    i = it->currentIndex;                                                   \
    if (i < (dim)) {                                                        \
...
#define SGLIB_DEFINE_HASHED_CONTAINER_PROTOTYPES(type, dim, hash_function)    \
  ...
  extern int sglib_hashed_##type##_delete_if_member(type *table[dim],         \
                                                    type *elem, type **memb); \
  extern int sglib_hashed_##type##_is_member(type *table[dim], type *elem);   \
  extern type *sglib_hashed_##type##_find_member(type *table[dim],            \
                                                 type *elem);                 \
...
#define SGLIB_DEFINE_HASHED_CONTAINER_FUNCTIONS(type, dim, hash_function)   \
  ...
  int sglib_hashed_##type##_delete_if_member(type *table[dim], type *elem,  \
                                             type **memb) {                 \
    unsigned i;                                                             \
    i = ((unsigned)hash_function(elem)) % (dim);                            \
...
#define SGLIB_DEFINE_LIST_PROTOTYPES(type, comparator, next)                  \
  ...
  extern int sglib_##type##_add_if_not_member(type **list, type *elem,        \
                                              type **member);                 \
  extern void sglib_##type##_concat(type **first, type *second);              \
  extern void sglib_##type##_delete(type **list, type *elem);                 \
  extern int sglib_##type##_delete_if_member(type **list, type *elem,         \
                                             type **member);                  \
  ...
  extern type *sglib_##type##_it_init(struct sglib_##type##_iterator *it,     \
                                      type *list);                            \
  extern type *sglib_##type##_it_init_on_equal(                               \
      struct sglib_##type##_iterator *it, type *list,                         \
      int (*subcomparator)(type *, type *), type *equalto);                   \
...
#define SGLIB_DEFINE_LIST_FUNCTIONS(type, comparator, next)                    \
  ...
  int sglib_##type##_add_if_not_member(type **list, type *elem,                \
                                       type **member) {                        \
    SGLIB_LIST_ADD_IF_NOT_MEMBER(type, *list, elem, comparator, next,          \
                                 *member);                                     \
    return (*member == NULL);                                                  \
  }                                                                            \
  ...
  int sglib_##type##_delete_if_member(type **list, type *elem,                 \
                                      type **member) {                         \
  ...
  type *sglib_##type##_it_init(struct sglib_##type##_iterator *it,             \
                               type *list) {                                   \
...
#define SGLIB_DEFINE_SORTED_LIST_FUNCTIONS(type, comparator, next)             \
  ...
  int sglib_##type##_add_if_not_member(type **list, type *elem,                \
                                       type **member) {                        \
    SGLIB_SORTED_LIST_ADD_IF_NOT_MEMBER(type, *list, elem, comparator, next,   \
                                        *member);                              \
    return (*member == NULL);                                                  \
  }                                                                            \
  void sglib_##type##_add(type **list, type *elem) {                           \
  ...
  int sglib_##type##_delete_if_member(type **list, type *elem,                 \
                                      type **member) {                         \
    SGLIB_SORTED_LIST_DELETE_IF_MEMBER(type, *list, elem, comparator, next,    \
                                       *member);                               \
    return (*member != NULL);                                                  \
  }                                                                            \
  int sglib_##type##_len(type *list) {                                         \
...
#define SGLIB_DEFINE_DL_LIST_PROTOTYPES(type, comparator, previous, next)     \
  ...
  extern int sglib_##type##_add_if_not_member(type **list, type *elem,        \
                                              type **member);                 \
  extern int sglib_##type##_add_before_if_not_member(type **list, type *elem, \
                                                     type **member);          \
  extern int sglib_##type##_add_after_if_not_member(type **list, type *elem,  \
                                                    type **member);           \
  extern void sglib_##type##_concat(type **first, type *second);              \
  extern void sglib_##type##_delete(type **list, type *elem);                 \
  extern int sglib_##type##_delete_if_member(type **list, type *elem,         \
...
#define SGLIB_DEFINE_DL_LIST_FUNCTIONS(type, comparator, previous, next)     \
  ...
  int sglib_##type##_add_if_not_member(type **list, type *elem,              \
                                       type **member) {                      \
    SGLIB_DL_LIST_ADD_IF_NOT_MEMBER(type, *list, elem, comparator, previous, \
                                    next, *member);                          \
    return (*member == NULL);                                                \
  }                                                                          \
  int sglib_##type##_add_after_if_not_member(type **list, type *elem,        \
                                             type **member) {                \
    SGLIB_DL_LIST_ADD_AFTER_IF_NOT_MEMBER(type, *list, elem, comparator,     \
                                          previous, next, *member);          \
    return (*member == NULL);                                                \
  }                                                                          \
  int sglib_##type##_add_before_if_not_member(type **list, type *elem,       \
                                              type **member) {               \
    SGLIB_DL_LIST_ADD_BEFORE_IF_NOT_MEMBER(type, *list, elem, comparator,    \
                                           previous, next, *member);         \
    return (*member == NULL);                                                \
  }                                                                          \
  void sglib_##type##_concat(type **first, type *second) {                   \
  ...
  int sglib_##type##_delete_if_member(type **list, type *elem,               \
                                      type **member) {                       \
    SGLIB_DL_LIST_DELETE_IF_MEMBER(type, *list, elem, comparator, previous,  \
                                   next, *member);                           \
    return (*member != NULL);                                                \
  }                                                                          \
  int sglib_##type##_is_member(type *list, type *elem) {                     \
  ...
  type *sglib_##type##_find_member(type *list, type *elem) {                 \
    type *result;                                                            \
    SGLIB_DL_LIST_FIND_MEMBER(type, list, elem, comparator, previous, next,  \
                              result);                                       \
    return (result);                                                         \
  }                                                                          \
...
#define SGLIB___RBTREE_FIX_INSERTION_DISCREPANCY(type, tree, leftt, rightt,   \
                                                 bits, RED, BLACK)            \
  {                                                                           \
    ...
    if (t->rightt != NULL && SGLIB___GET_VALUE(t->rightt->bits) == RED) {     \
      if (SGLIB___GET_VALUE(tl->bits) == RED) {                               \
        if ((tl->leftt != NULL &&                                             \
             SGLIB___GET_VALUE(tl->leftt->bits) == RED) ||                    \
            (tl->rightt != NULL &&                                            \
             SGLIB___GET_VALUE(tl->rightt->bits) == RED)) {                   \
...
#define SGLIB___RBTREE_FIX_DELETION_DISCREPANCY(type, tree, leftt, rightt, \
                                                bits, RED, BLACK, res)     \
  {                                                                        \
...
#define SGLIB_DEFINE_RBTREE_FUNCTIONS_GENERAL(type, left, right, bits,         \
                                              comparator, RED, BLACK)          \
  static void sglib___##type##_fix_left_insertion_discrepancy(type **tree) {   \
    SGLIB___RBTREE_FIX_INSERTION_DISCREPANCY(type, tree, left, right, bits,    \
                                             RED, BLACK);                      \
  }                                                                            \
                                                                               \
  static void sglib___##type##_fix_right_insertion_discrepancy(type **tree) {  \
    SGLIB___RBTREE_FIX_INSERTION_DISCREPANCY(type, tree, right, left, bits,    \
                                             RED, BLACK);                      \
  }                                                                            \
                                                                               \
  static int sglib___##type##_fix_left_deletion_discrepancy(type **tree) {     \
    int res;                                                                   \
    SGLIB___RBTREE_FIX_DELETION_DISCREPANCY(type, tree, right, left, bits,     \
                                            RED, BLACK, res);                  \
    return (res);                                                              \
  }                                                                            \
                                                                               \
  static int sglib___##type##_fix_right_deletion_discrepancy(type **tree) {    \
    int res;                                                                   \
    SGLIB___RBTREE_FIX_DELETION_DISCREPANCY(type, tree, left, right, bits,     \
                                            RED, BLACK, res);                  \
    return (res);                                                              \
  }                                                                            \
                                                                               \
      ...
      if (cmp < 0 || (cmp == 0 && elem < t)) {                                 \
        ...
        if (SGLIB___GET_VALUE(t->bits) == BLACK)                               \
          sglib___##type##_fix_right_insertion_discrepancy(tree);              \
                                                                               ...
                                                                               \
  static int sglib___##type##_delete_rightmost_leaf(type **tree,               \
                                                    type **theLeaf) {          \
    type *t;                                                                   \
    ...
    if (t->right == NULL) {                                                    \
      *theLeaf = t;                                                            \
      ...
      deepDecreased =                                                          \
          sglib___##type##_delete_rightmost_leaf(&t->right, theLeaf);          \
      if (deepDecreased)                                                       \
  ...
  int sglib___##type##_delete_recursive(type **tree, type *elem) {             \
    ...
    if (t == NULL) {                                                           \
      assert(0 &&                                                              \
             "The element to delete not found in the tree,  use "              \
      ...
      if (cmp < 0 || (cmp == 0 && elem < t)) {                                 \
        ...
        assert(elem == t &&                                                    \
               "Deleting an element which is non member of the tree, use "     \
               "'delete_if_member'" != NULL);                                  \
        if (t->left == NULL) {                                                 \
          if (t->right == NULL) {                                              \
            /* a leaf, delete, it; */                                          \
          ...
          deepDecreased =                                                      \
              sglib___##type##_delete_rightmost_leaf(&t->left, &theLeaf);      \
          theLeaf->left = t->left;                                             \
  ...
  int sglib_##type##_len(type *t) {                                            \
    ...
    return (n);                                                                \
                                                                               ...
                                                                               \
  void sglib__##type##_it_compute_current_elem(                                \
        ...
        if (it->pass[i] >= 2) {                                                \
          ...
          if (eqt != NULL) {                                                   \
            if (subcomparator == NULL) {                                       \
              SGLIB___BIN_TREE_FIND_MEMBER(type, s, eqt, left, right,          \
                                           comparator, s);                     \
            } else {                                                           \
              SGLIB___BIN_TREE_FIND_MEMBER(type, s, eqt, left, right,          \
                                           subcomparator, s);                  \
  ...
  type *sglib__##type##_it_init(                                               \
      struct sglib_##type##_iterator *it, type *tree, int order,               \
      int (*subcomparator)(type *, type *), type *equalto) {                   \
    type *t;                                                                   \
    assert(it != NULL);                                                        \
    it->order = order;                                                         \
    ...
    if (equalto == NULL) {                                                     \
      ...
      if (subcomparator == NULL) {                                             \
        SGLIB___BIN_TREE_FIND_MEMBER(type, tree, equalto, left, right,         \
                                     comparator, t);                           \
      } else {                                                                 \
        SGLIB___BIN_TREE_FIND_MEMBER(type, tree, equalto, left, right,         \
                                     subcomparator, t);                        \
  ...
  type *sglib_##type##_it_init_preorder(struct sglib_##type##_iterator *it,    \
                                        type *tree) {                          \
    return (sglib__##type##_it_init(it, tree, 0, NULL, NULL));                 \
  }                                                                            \
  type *sglib_##type##_it_init_inorder(struct sglib_##type##_iterator *it,     \
                                       type *tree) {                           \
    return (sglib__##type##_it_init(it, tree, 1, NULL, NULL));                 \
  }                                                                            \
  type *sglib_##type##_it_init_postorder(struct sglib_##type##_iterator *it,   \
                                         type *tree) {                         \
    return (sglib__##type##_it_init(it, tree, 2, NULL, NULL));                 \
  }                                                                            \
  type *sglib_##type##_it_init_on_equal(                                       \
      struct sglib_##type##_iterator *it, type *tree,                          \
      int (*subcomparator)(type *, type *), type *equalto) {                   \
  ...
  type *sglib_##type##_it_next(struct sglib_##type##_iterator *it) {           \
    ...
    return (it->currentelem);                                                  \
                                                                               ...
                                                                               \
  static void sglib___##type##_consistency_check_recursive(                    \
      type *t, int *pathdeep, int cdeep) {                                     \
    if (t == NULL) {                                                           \
      if (*pathdeep < 0)                                                       \
        *pathdeep = cdeep;                                                     \
      else                                                                     \
      ...
      if (SGLIB___GET_VALUE(t->bits) == RED) {                                 \
        ...
        assert(t->right == NULL ||                                             \
               SGLIB___GET_VALUE(t->right->bits) == BLACK);                    \
        sglib___##type##_consistency_check_recursive(t->left, pathdeep,        \
                                                     cdeep);                   \
        sglib___##type##_consistency_check_recursive(t->right, pathdeep,       \
                                                     cdeep);                   \
      } else {                                                                 \
        sglib___##type##_consistency_check_recursive(t->left, pathdeep,        \
                                                     cdeep + 1);               \
...
#define SGLIB_DEFINE_RBTREE_PROTOTYPES(type, left, right, colorbit,           \
                                       comparator)                            \
  struct sglib_##type##_iterator {                                            \
    type *currentelem;                                                        \
  ...
  extern int sglib_##type##_add_if_not_member(type **tree, type *elem,        \
                                              type **memb);                   \
  extern void sglib_##type##_delete(type **tree, type *elem);                 \
  extern int sglib_##type##_delete_if_member(type **tree, type *elem,         \
                                             type **memb);                    \
  ...
  extern type *sglib_##type##_it_init(struct sglib_##type##_iterator *it,     \
                                      type *tree);                            \
  extern type *sglib_##type##_it_init_preorder(                               \
      struct sglib_##type##_iterator *it, type *tree);                        \
  extern type *sglib_##type##_it_init_inorder(                                \
      struct sglib_##type##_iterator *it, type *tree);                        \
  extern type *sglib_##type##_it_init_postorder(                              \
      struct sglib_##type##_iterator *it, type *tree);                        \
  extern type *sglib_##type##_it_init_on_equal(                               \
      struct sglib_##type##_iterator *it, type *tree,                         \
      int (*subcomparator)(type *, type *), type *equalto);                   \
...
#define SGLIB_DEFINE_RBTREE_FUNCTIONS(type, left, right, colorbit, comparator) \
  SGLIB_DEFINE_RBTREE_FUNCTIONS_GENERAL(type, left, right, colorbit,           \
                                        comparator, 1, 0)
...
#define SGLIB_SAFE_NUMERIC_COMPARATOR(x, y) \
  (((x) > (y) ? 1 : ((x) < (y) ? -1 : 0)))
...
#define SGLIB_REVERSE_NUMERIC_COMPARATOR(x, y) \
  SGLIB_SAFE_REVERSE_NUMERIC_COMPARATOR(x, y)
