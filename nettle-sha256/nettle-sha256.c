...
#define MD_PAD(ctx, size, f)                                                   \
  do {                                                                         \
    ...
    if (__md_i > (sizeof((ctx)->block) -                                       \
                  (size))) { /* No room for length in this block. Process it   \
                                and pad with another one */                                                       \
      memset((ctx)->block + __md_i, 0, sizeof((ctx)->block) - __md_i);         \
                                                                               \
      f((ctx), (ctx)->block);                                                  \
      __md_i = 0;                                                              \
    }                                                                          \
    memset((ctx)->block + __md_i, 0, sizeof((ctx)->block) - (size)-__md_i);    \
                                                                               \
  } while (0)
...
static void sha256_write_digest(struct sha256_ctx *ctx, size_t length,
                                uint8_t *digest) {
  uint64_t bit_count;
...
#define _NETTLE_HASH(name, NAME)                                               \
  {                                                                            \
#name, sizeof(struct name##_ctx), NAME##_DIGEST_SIZE, NAME##_BLOCK_SIZE,   \
...
void initialise_benchmark(void) { memset(buffer, 0, sizeof(buffer)); }
